...
func TestScrapeSuccess(t *testing.T) {
	...
	if len(rates) != 2 {
		t.Fatalf("expected exactly 2 rates (USD + EUR), got %d", len(rates))
	}
	...
	for _, r := range rates {
		if !r.ScrapedAt.Equal(expectedDate) {
			t.Errorf("rate %s: ScrapedAt = %v, want %v", r.Currency, r.ScrapedAt, expectedDate)
		}
	}
}
