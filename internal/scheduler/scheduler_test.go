...
func TestScheduler_Initialization(t *testing.T) {
	...
	if sched.scrapeHour != 8 {
		t.Errorf("expected scrapeHour to be 8, got %d", sched.scrapeHour)
	}
...
func TestScheduler_RetryLogic_Failure(t *testing.T) {
	...
	sched := NewScheduler(mu, 8)
	// Use small delays to speed up test execution
...
func TestScheduler_RetryLogic_ContextCancelled(t *testing.T) {
	...
	sched := NewScheduler(mu, 8)
	// Use longer backoffs so cancel interrupts the delay immediately
	sched.backoffDelays = []time.Duration{
		10 * time.Second,
		10 * time.Second,
		10 * time.Second,
	...
	ctx, cancel := context.WithCancel(context.Background())
	// Cancel the context asynchronously after a very short time
	go func() {
